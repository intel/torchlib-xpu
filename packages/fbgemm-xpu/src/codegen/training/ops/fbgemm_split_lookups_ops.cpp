...
////////////////////////////////////////////////////////////////////////////////
// SYCL PORT MAPPING TO FBGEMM CUDA SOURCE - AUTOGRAD FUNCTIONS
////////////////////////////////////////////////////////////////////////////////
//
// This file contains SYCL ports of FBGEMM split embedding autograd functions
...
//   Generated File:
//   fbgemm_gpu/_skbuild/linux-x86_64-3.10/cmake-build/gen_embedding_split_rowwise_adagrad_pt2_autograd.cpp
//
// AUTOGRAD FUNCTION MAPPING:
//   SplitNoBagLookupFunction_rowwise_adagrad_Op_pt2 (SYCL)
//     → SplitNoBagLookupFunction_rowwise_adagrad_Op_pt2 (CUDA)
//
// HOST FUNCTION MAPPING:
//   split_embedding_codegen_lookup_rowwise_adagrad_function_pt2_xpu
//     → split_embedding_codegen_lookup_rowwise_adagrad_function_pt2 (CUDA)
//     CUDA File:
//     fbgemm_gpu/_skbuild/linux-x86_64-3.10/cmake-build/gen_embedding_split_rowwise_adagrad_pt2_autograd.cpp
//
// DESCRIPTION:
...
//   - Forward operator:
//   fbgemm::split_embedding_nobag_codegen_forward_unweighted_pt2_wrapper
//   - Backward operator:
//   fbgemm::split_embedding_nobag_backward_codegen_rowwise_adagrad_unweighted_pt2_wrapper
//   - Autograd function:
...
//
// NOTE: VBE (Variable Batch Embedding) and GWD (Global Weight Decay) features
...
#define GET_OPTIONAL_TENSOR_VALUE(name, empty_tensor) \
    name.has_value() ? name.value() : empty_tensor;

////////////////////////////////////////////////////////////////////////////////
// SplitNoBagLookupFunction_rowwise_adagrad_Op_pt2 - Autograd Function
////////////////////////////////////////////////////////////////////////////////
//
// CUDA SOURCE MAPPING:
//   CUDA Class: SplitNoBagLookupFunction_rowwise_adagrad_Op_pt2
//   CUDA File:
//   fbgemm_gpu/_skbuild/linux-x86_64-3.10/cmake-build/gen_embedding_split_rowwise_adagrad_pt2_autograd.cpp
//   CUDA Template:
//   fbgemm_gpu/codegen/training/pt2/embedding_split_host_pt2_autograd_template.cpp
//
// DESCRIPTION:
//   PyTorch autograd function for split embeddings with rowwise Adagrad.
//   Implements forward() and backward() methods for automatic differentiation.
//   Supports PT2 compilation with SymInt types and compile guards.
//
// FORWARD PASS:
//   Dispatches to:
//   fbgemm::split_embedding_nobag_codegen_forward_unweighted_pt2_cuda_wrapper
//   XPU Implementation: split_embedding_nobag_forward_unweighted_xpu
//
// BACKWARD PASS:
//   Dispatches to:
//   fbgemm::split_embedding_nobag_backward_codegen_rowwise_adagrad_unweighted_pt2_cuda_wrapper
//   XPU Implementation:
...
class SplitNoBagLookupFunction_rowwise_adagrad_Op_pt2
 ...
 public:
    ...
    forward(torch::autograd::AutogradContext* ctx,
            ...
            const int64_t output_dtype, const at::TensorList weights,
            ...
            std::vector<int64_t> aux_int, std::vector<double> aux_float,
            c10::List<bool> aux_bool, at::TensorList momentum1,
            Tensor learning_rate_tensor, std::vector<int64_t> optim_int,
        ...
        if (is_annotate_trace_enabled) {
            ...
            record_trace = profiler::record_function_enter_new("split_tbe_fwd" +
                                                               op_annotation);
            ctx->saved_data["op_annotation"] = op_annotation;
        }
        // NOTE: The `local_uvm_cache_stats` variable held by the nn.Module has
        // dtype int32_t
        at::TensorOptions uvm_options = weights_host.numel() > 0
                                            ? weights_host.options()
                                            : weights_dev.options();
        const auto uvm_cache_stats = GET_OPTIONAL_TENSOR_VALUE(
            aux_tensor[IDX_UVM_CACHE_STATS],
            at::empty({0}, uvm_options.dtype(at::kInt)));
        TORCH_CHECK(aux_tensor[IDX_LXU_CACHE_LOCATIONS].has_value(),
                    "lxu_cache_locations should have value.");
        const auto lxu_cache_locations =
            aux_tensor[IDX_LXU_CACHE_LOCATIONS].value();
        ...
        const auto info_B_num_bits =
            static_cast<int32_t>(aux_int[IDX_INFO_B_NUM_BITS]);
        const auto info_B_mask =
            static_cast<uint32_t>(aux_int[IDX_INFO_B_MASK]);
        TORCH_SYM_CHECK(max_B_.sym_le(info_B_mask),
                        "Not enough bits to accommodate B");  // vbe

        // Setting learning rate tensor with `.fill_()` breaks apf_dlrm bento
        // kernel with `RuntimeError: one of the variables needed for gradient
        // computation has been modified by an inplace operation.` This is
        // because if a tensor is saved for backward and it is mutated later,
        // this can cause correctness problems. Since the forward compute and
        // backward compute see different data values for this tensor. To work
        // around, we pass the cloned tensor instead the mutated tensor
        Tensor learning_rate_tensor_cloned = learning_rate_tensor.clone();

        ctx->save_for_backward(
            {weights_host, weights_dev, weights_uvm, weights_lxu_cache,
             weights_placements, weights_offsets, hash_size_cumsum, indices,
             offsets, lxu_cache_locations, momentum1_host, momentum1_dev,
             momentum1_uvm, momentum1_placements, momentum1_offsets,
             learning_rate_tensor_cloned});
        ctx->saved_data["D"] = D;
        ctx->saved_data["total_hash_size_bits"] = total_hash_size_bits;
        ctx->saved_data["gradient_clipping"] =
            static_cast<bool>(aux_bool[IDX_GRADIENT_CLIPPING]);
        ctx->saved_data["max_gradient"] = aux_float[IDX_MAX_GRADIENT];
        ctx->saved_data["stochastic_rounding"] =
            static_cast<bool>(aux_bool[IDX_STOCHASTIC_ROUNDING]);
        ...
        ctx->saved_data["use_uniq_cache_locations_bwd"] =
            static_cast<bool>(aux_bool[IDX_USE_UNIQ_CACHE_LOCATIONS_BWD]);
        ctx->saved_data["use_homogeneous_placements"] =
            static_cast<bool>(aux_bool[IDX_USE_HOMOGENEOUS_PLACEMENTS]);
        ...
        static auto embedding_forward_op =
            torch::Dispatcher::singleton()
                .findSchemaOrThrow(
                    "fbgemm::split_embedding_nobag_codegen_forward_unweighted_"
                    "pt2_wrapper",
                    "")
                .typed<Tensor(
        ...
        auto output = embedding_forward_op.call(
            weights_host, flatten_weights_dev, weights_uvm, weights_lxu_cache,
            weights_placements, weights_offsets, D, hash_size_cumsum, indices,
            offsets, lxu_cache_locations, uvm_cache_stats, is_experimental,
            output_dtype);
    ...
    backward(torch::autograd::AutogradContext* ctx,
        ...
        auto stochastic_rounding =
            ctx->saved_data["stochastic_rounding"].toBool();
        ...
        auto grad_output =
            gradient_clipping
                ? clamp(grad_outputs[0], -max_gradient, max_gradient)
                : grad_outputs[0];
        // nobag
        ...
        static auto embedding_unweighted_backward_op =
            torch::Dispatcher::singleton()
                .findSchemaOrThrow(
                    "fbgemm::split_embedding_nobag_backward_codegen_rowwise_"
                    "adagrad_unweighted_pt2_wrapper",
                    "")
                .typed<Tensor(
                    ...
                    const bool /*use_homogeneous_placements*/, Tensor, Tensor,
                    Tensor, Tensor, Tensor, Tensor, double, double, int64_t,
                    double)>();
        ...
        grad_weights_dev = embedding_unweighted_backward_op.call(
            ...
            momentum1_offsets, learning_rate_tensor, eps, weight_decay,
            weight_decay_mode, max_norm);

        if (is_annotate_trace_enabled) {
            record_trace->record.end();
        }

        // Number of returned gradients have to match the input to Autograd's
        // forward The number of items in the tensorlist differ between devices
...
//
// CUDA SOURCE MAPPING:
//   CUDA Function: split_embedding_codegen_lookup_rowwise_adagrad_function_pt2
//   CUDA File:
//   fbgemm_gpu/_skbuild/linux-x86_64-3.10/cmake-build/gen_embedding_split_rowwise_adagrad_pt2_autograd.cpp
//   CUDA Template:
//   fbgemm_gpu/codegen/training/pt2/embedding_split_host_pt2_autograd_template.cpp
//
// DESCRIPTION:
//   Main entry point for split embedding lookups with rowwise Adagrad
//   optimizer. Handles both pooled (bag) and non-pooled (nobag/sequence)
//   embedding lookups. Routes to appropriate autograd function based on
//   pooling_mode parameter.
//
// SUPPORTED FEATURES (XPU):
//   - No-bag (sequence) embeddings: PoolingMode::NONE
//   - Rowwise Adagrad optimizer
//   - PT2 compilation support
//
// NOT SUPPORTED (XPU):
//   - VBE (Variable Batch Embedding)
//   - GWD (Global Weight Decay)
//   - Pooled embeddings (SUM, MEAN pooling modes)
//
// PYTORCH DISPATCHER:
//   Registered as:
//   fbgemm::split_embedding_codegen_lookup_rowwise_adagrad_function_pt2
//   Dispatch Key: AutogradXPU
//
////////////////////////////////////////////////////////////////////////////////
Tensor split_embedding_codegen_lookup_rowwise_adagrad_function_pt2_xpu(
    const Tensor& placeholder_autograd_tensor, const at::TensorList weights,
    ...
    const std::vector<int64_t>& aux_int, const std::vector<double>& aux_float,
    c10::List<bool> aux_bool, at::TensorList momentum1,
    Tensor learning_rate_tensor, std::vector<int64_t> optim_int,
    std::vector<double> optim_float, const c10::SymInt max_B = -1,
    ...
    if (aux_bool[IDX_APPLY_GLOBAL_WEIGHT_DECAY] && optim_float[1] > 0) {
        // not vbe and gwd
        TORCH_CHECK(false,
                    "Global weight decay is not supported yet in SYCL backend");
    }
    ...
    if (static_cast<PoolingMode>(pooling_mode) == PoolingMode::NONE) {
        ...
        return SplitNoBagLookupFunction_rowwise_adagrad_Op_pt2::apply(
            ...
            hash_size_cumsum, total_hash_size_bits, indices, offsets,
            aux_tensor, aux_int, aux_float, aux_bool, momentum1,
            learning_rate_tensor, optim_int, optim_float)[0];
    } else {
        TORCH_CHECK(
            false,
            "split_embedding_codegen_lookup_rowwise_adagrad_function_pt2 is "
            "not implemented yet for pooled case in SYCL backend");
    }
}
